...
export const AddToCartModal: React.FC<AddToCartModalProps> = ({
  ...
  const handleAddToCart = async () => {
    const totalItemsToAdd = Object.entries(quantities).filter(
      ([_, qty]) => qty > 0,
    );
          ...
          >
            ...
            <div className="flex items-start justify-between mb-8">
              <div className="space-y-0.5">
                <Typography
                  variant="detail"
                  className="text-zinc-400 tracking-[0.2em] font-medium leading-none mb-1"
                >
                  CARRITO
                </Typography>
                <Typography
                  variant="h2"
                  className="text-xl sm:text-2xl font-black text-black tracking-tighter leading-none"
                >
                  Añadir al Carrito
                </Typography>
              </div>
            ...
            <div className="mb-8 flex gap-5 items-center">
              ...
              <div className="flex flex-col gap-0.5">
                <Typography
                  variant="detail"
                  className="text-zinc-400 tracking-wider overflow-visible"
                >
                  {product.vendor}
                </Typography>
                <Typography
                ...
                <Typography
                  variant="body"
                  className="text-base font-black text-black tracking-tighter"
                >
            ...
            <div className="mb-10">
              <Typography
                variant="small"
                className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-6 block"
              >
              ...
              {hasVariants ? (
                <div className="space-y-4">
                  {product.variants!.map((variant) => {
                    ...
                    return (
                      <div
                        key={variant.id}
                        className={`flex items-center justify-between py-4 border-b border-zinc-50 transition-all duration-300 ${quantity > 0 ? "bg-zinc-50/50 -mx-4 px-4 rounded-2xl border-transparent" : ""}`}
                      >
                        <div className="flex flex-col">
                          <span
                            className={`text-xs font-black tracking-widest uppercase transition-colors ${quantity > 0 ? "text-black" : "text-zinc-500"}`}
                          >
                            {variant.title}
                          </span>
                          <span className="text-[10px] text-zinc-400 font-bold uppercase mt-0.5 tracking-widest">
                            $
                            {(
                              variant.prices?.[0]?.amount || product.price
                            ).toLocaleString()}
                          </span>
                          {isOutOfStock ? (
                            <span className="text-[9px] text-red-400 font-bold uppercase mt-1 tracking-widest">
                              Agotado
                            </span>
                          ) : (
                            <span className="text-[9px] text-zinc-300 font-bold uppercase mt-1 tracking-widest">
                              Stock: {stockQuantity}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-1 bg-white border border-zinc-100 p-1 shadow-sm">
                          ...
                          <span
                            className={`text-[11px] font-black min-w-[30px] text-center ${quantity > 0 ? "text-black" : "text-zinc-300"}`}
                          >
                          ...
                          <button
                            onClick={() =>
                              updateVariantQuantity(
                                variant.id,
                                Math.min(stockQuantity, quantity + 1),
                              )
                            }
                ...
                (() => {
                  const stockQuantity =
                    product.variants?.[0]?.inventory_items?.[0]?.inventory
                      ?.location_levels?.[0]?.available_quantity ?? 0;
                  const isOutOfStock = stockQuantity <= 0;
                  ...
                  return (
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2 bg-white border border-zinc-100 p-1.5 shadow-sm">
                        <button
                          onClick={() =>
                            updateVariantQuantity(product.id, quantity - 1)
                          }
                        ...
                        <span
                          className={`text-lg font-black min-w-[40px] text-center ${isOutOfStock ? "text-zinc-200" : "text-black"}`}
                        >
                          {isOutOfStock ? 0 : quantity}
                        </span>
                        <button
                      ...
                      {isOutOfStock ? (
                        <span className="text-[10px] text-red-400 font-bold uppercase tracking-[0.2em]">
                          Agotado
                        </span>
                      ) : (
                        <span className="text-[10px] text-zinc-300 font-bold uppercase tracking-[0.2em]">
                          Stock: {stockQuantity}
                        </span>
                      )}
                    </div>
            ...
            <div className="mb-10 space-y-4 pt-6 border-t border-zinc-100">
              <div className="flex justify-between items-end">
                <div className="space-y-1">
                  <span className="text-[10px] uppercase font-bold text-zinc-300 tracking-[0.2em] block leading-none">
                    TOTAL UNIDADES
                  </span>

...
/**
 * WallOfLove: Muestra 3 testimonios a la vez, rota automáticamente con
 * una animación de crossfade (fade + scale) igual a la del carrusel de logos.
 */
export const WallOfLove: React.FC = () => {
  ...
  // ── Auto-rotation (cada 5 s, igual que el carrusel de logos) ─────────────
  useEffect(() => {
  ...
  const visibleReviews = useMemo(() => {
    ...
    return reviews.slice(currentIndex, currentIndex + ITEMS_PER_PAGE);
  ...
  return (
    <section
      ...
      className="py-24 sm:py-32 px-4 sm:px-6 overflow-hidden"
      id="wall-of-love"
    >
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center sm:items-end gap-10 mb-20 px-2 sm:px-4 text-center sm:text-left">
          <div className="space-y-4 max-w-2xl">
            ...
            <Typography
              variant="h2"
              className="text-3xl sm:text-5xl font-black uppercase tracking-tighter italic text-neutral-950 leading-tight"
            >
          ...
          <div className="flex items-center gap-4 sm:gap-6 bg-white border border-neutral-100 px-6 sm:px-8 py-4 sm:py-5 rounded-2xl sm:rounded-4xl shadow-2xl shadow-black/5">
            ...
            <div className="flex flex-col">
              <span className="text-2xl sm:text-3xl font-black italic text-neutral-950 leading-none">
                {averageRating}
              </span>
              <span className="text-[8px] font-black uppercase text-neutral-400 tracking-widest mt-1">
                PROMEDIO GLOBAL
              </span>
            </div>
          </div>
        ...
        {reviews.length === 0 ? (
          <div className="text-center py-24 rounded-4xl border border-dashed border-neutral-200">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-neutral-300 max-w-xs mx-auto">
              Aún no hay testimonios. <br />¡Sé el primero en calificar!
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-10">

            {/* Crossfade grid — igual que LogoCarousel */}
                ...
                >
                  {visibleReviews.map((review) => {
                    ...
                    return (
                      <div
                        key={review.id}
                        className={`bg-white rounded-4xl p-8 sm:p-10 border border-neutral-100 shadow-xl shadow-black/5 relative overflow-hidden group transition-all duration-500 hover:-translate-y-2 flex flex-col justify-between ${
                      ...
                      >
                        ...
                        <div className="space-y-6 relative z-10 flex-1">
                          <div className="flex justify-between items-start">
                            <div className="flex gap-1 text-amber-400">
                              {Array.from({ length: 5 }).map((_, i) => (
                                <Star
                                  ...
                                  className={
                                    i < review.rating
                                      ? 'drop-shadow-[0_0_5px_rgba(251,191,36,0.2)]'
                                      : 'text-neutral-100'
                                  }
                                />
                              ))}
                            ...
                            <span
                              className={`text-[8px] sm:text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${
                                isOwner
                                  ? 'text-accent bg-accent/5 border-accent/10'
                                  : 'text-emerald-600 bg-emerald-50 border-emerald-100'
                              }`}
                            >
                              {isOwner && <span className="inline-block mr-1">●</span>}
                              Experiencia Web
                            </span>
                          </div>
                          ...
                          <Typography
                            variant="body"
                            className="text-xs sm:text-sm text-neutral-700 leading-relaxed font-semibold italic"
                          >
                        ...
                        <div className="mt-8 pt-8 border-t border-neutral-100 flex items-center justify-between relative z-10">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="overflow-hidden">
                              <Typography
                                variant="h4"
                                className="text-[10px] sm:text-xs font-black uppercase text-neutral-950 leading-none mb-1 truncate"
                              >
                                {review.customer_name || 'Usuario'}
                              </Typography>
                              <p className="text-[8px] sm:text-[9px] font-bold text-neutral-400 tracking-widest uppercase truncate">

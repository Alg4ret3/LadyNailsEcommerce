          ...
          >
            ...
            <div className="p-4 sm:p-6 md:p-8 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
                ...
                <span className="text-[8px] sm:text-[9px] bg-black/5 text-foreground/20 px-1.5 sm:px-2 py-0.5 rounded-full font-bold shrink-0">
                  {cartItems.length}
                </span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 no-scrollbar">
              {cartItems.length === 0 ? (
                ...
                cartItems.map((item) => (
                  <div key={`${item.id}-${item.size}`} className="flex gap-4 sm:gap-6 group relative border-b border-black/10 pb-8 last:border-0 min-w-0">
                    <div className="relative w-16 sm:w-20 aspect-square bg-black/[0.03] rounded-sm overflow-hidden shrink-0">
                      {item.image ? (
                      ...
                      <div className="space-y-1 min-w-0">
                        <div className="flex justify-between items-start gap-4 min-w-0">
                          ...
                          <button 
                            ...
                            className="text-foreground/10 hover:text-red-500 transition-colors"
                          >
            ...
            {cartItems.length > 0 && (
              <div className="p-8 space-y-6 bg-white">
                <div className="space-y-4">
                  <div className="flex justify-between items-center border-t border-black/30 pt-6">
                    <span className="text-[10px] text-foreground uppercase tracking-[0.2em] font-medium">Total</span>
                    <span className="text-xl font-semibold tracking-tighter text-foreground">${totalAmount.toLocaleString()}</span>
                  </div>
                  
                  <div className="flex items-center gap-2">
                    <div className="h-[1px] flex-1 bg-black/10" />
                    <span className="text-[8px] uppercase tracking-[0.3em] text-foreground">Envío calculado al finalizar</span>
                    <div className="h-[1px] flex-1 bg-black/10" />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full bg-black text-white py-4 text-[10px] uppercase tracking-[0.25em] hover:bg-black transition-all font-medium"
                  >
                    Finalizar Compra
                  </button>
                  ...
                  <button 
                     onClick={() => setIsCartOpen(false)}
                     className="w-full text-[9px] uppercase tracking-[0.2em] text-foreground hover:text-foreground transition-colors py-2"
                  >
                    Seguir comprando
                  </button>
                </div>

...
export default function AddressesPage() {
  const { user, createAddress, updateAddress, deleteAddress } = useUser();
  ...
  // Original snapshot — used to detect changes
  const [originalForm, setOriginalForm] = React.useState<ReturnType<typeof emptyForm> | null>(null);

  const [addressForm, setAddressForm] = React.useState(emptyForm(user));

  // Detect if the form has changed vs the original
  const hasChanges = React.useMemo(() => {
    if (!originalForm) return true; // new address — always enabled
    return JSON.stringify(addressForm) !== JSON.stringify(originalForm);
  }, [addressForm, originalForm]);

  const resetAddressForm = () => {
    setAddressForm(emptyForm(user));
  ...
  // The address being edited (for the preview card)
  const editingAddr = editingAddressId
    ? user.addresses?.find((a: any) => a.id === editingAddressId)
    : null;
    ...
    <div className="space-y-8">
      ...
      {isAddingAddress && (
        <div className="space-y-6">
          ...
          {/* ── Dirección actual (solo al editar) ─────────────────────── */}
          {editingAddr && (
            <div className="bg-neutral-50 border-l-4 border-l-black border border-neutral-200 p-6 sm:p-8">
              <p className="text-[9px] font-black uppercase tracking-[0.25em] text-neutral-400 mb-4">Dirección actual guardada</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-3">

                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-neutral-300 mb-0.5">Etiqueta</p>
                  <p className="text-sm font-black text-neutral-950">{editingAddr.label || '—'}</p>
                </div>

                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-neutral-300 mb-0.5">Contacto</p>
                  <p className="text-sm font-bold text-neutral-700">{editingAddr.firstName} {editingAddr.lastName}</p>
                </div>

                <div className="sm:col-span-2">
                  <p className="text-[8px] font-black uppercase tracking-widest text-neutral-300 mb-0.5">Dirección de Envío</p>
                  <p className="text-sm font-bold text-neutral-700">{editingAddr.street || '—'}</p>
                  {editingAddr.apartment && <p className="text-xs text-neutral-400">{editingAddr.apartment}</p>}
                </div>

                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-neutral-300 mb-0.5">Ciudad / Departamento</p>
                  <p className="text-sm font-bold text-neutral-700">
                    {editingAddr.city || '—'}{editingAddr.province ? `, ${editingAddr.province}` : ''}
                  </p>
                </div>

                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-neutral-300 mb-0.5">Teléfono</p>
                  <p className="text-sm font-bold text-neutral-700">{editingAddr.phone || '—'}</p>
                </div>

                {editingAddr.postalCode && (
                  <div>
                    <p className="text-[8px] font-black uppercase tracking-widest text-neutral-300 mb-0.5">Código Postal</p>
                    <p className="text-sm font-bold text-neutral-700">{editingAddr.postalCode}</p>
                  </div>
                )}
          ...
          <div className="bg-white border-4 border-black p-8 sm:p-12 space-y-10 shadow-2xl animate-in fade-in slide-in-from-top-8 duration-500">
            ...
            <form onSubmit={handleSaveAddress} className="space-y-10">
              ...
              <div className="pt-6 border-t border-neutral-100 flex items-center gap-6">
                <Button
                  type="submit"
                  label={isActionPending ? 'Procesando...' : (editingAddressId ? 'Actualizar Información' : 'Registrar Dirección')}
                  disabled={isActionPending || !hasChanges}
                  className={`px-12 transition-opacity ${!hasChanges ? 'opacity-40 cursor-not-allowed' : ''}`}
                />
          ...
          <div key={addr.id} className="bg-white border border-neutral-200 p-8 sm:p-10 space-y-6 group hover:border-black transition-all shadow-sm">
            ...
            <div className="space-y-4 pt-4 border-t border-neutral-50">
              <div className="space-y-1">
                <Typography variant="body" className="text-sm font-bold text-black leading-tight">{addr.street}</Typography>

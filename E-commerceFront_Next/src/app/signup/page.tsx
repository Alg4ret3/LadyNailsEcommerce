...
export default function SignupPage() {
  ...
  const { signupStep1, signupStep2, signupStep3 } = useUser();
  ...
  const handleStep2 = async (e: React.FormEvent) => {
    e.preventDefault();
    const success = await signupStep2(code);
    if (success) {
      setStep(3);
      setError('');
    } else {
        ...
        <div className="max-w-2xl w-full bg-white border border-slate-200 p-8 sm:p-16 space-y-12 shadow-xl">
           ...
           {step === 1 && (
             <form className="space-y-8" onSubmit={handleStep1}>
                ...
                <Button type="submit" label="Recibir Código" className="w-full py-5" />
                <div className="text-center">
                   <Typography variant="small">¿Ya tiene una cuenta? <Link href="/login" className="text-slate-900 font-black underline underline-offset-4">Ingresar aquí</Link></Typography>
                </div>
             </form>
           )}
           ...
           {step === 2 && (
             <form className="space-y-8" onSubmit={handleStep2}>
                ...
                <button onClick={() => setStep(1)} className="w-full text-xs uppercase font-black tracking-widest text-slate-400 hover:text-slate-900 transition-colors">Volver al paso anterior</button>
             </form>
           )}
           ...
           {step === 3 && (
             <form className="space-y-12" onSubmit={handleStep3}>
                ...
                <div className="flex flex-col items-center gap-6">
                   <Button type="submit" label="Finalizar Registro" className="w-full py-5" />
                </div>
             </form>
           )}
        </div>
